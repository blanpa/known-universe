  ...
  {#each groups as g, gi}
    <div class="ctl-group" class:closed={closed[gi]}>
      ...
      {#each g.items as d (d.id)}
        <div class="toggle" class:on={d.ui ? $timeBar : d.inv ? !($toggleState[d.id] ?? d.on) : ($toggleState[d.id] ?? d.on)} onclick={() => tgl(d.id)}>
